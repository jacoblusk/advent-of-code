...
[[nodiscard]] struct directory *process_line(char *line, struct directory *parent)
{
    char *token;
    const char *delims = " ";
...
[[nodiscard]] _Bool getline(FILE *input, char **line_out)
{
    char c;
...
[[nodiscard]] size_t get_directory_size(struct directory *directory)
{
    size_t file_sizes = 0;
...
[[nodiscard]] size_t total_conditional_sum_of_directories(struct directory *parent)
{
...
[[nodiscard]] size_t find_smallest_directory_to_delete(struct directory *parent, size_t required_size)
{
